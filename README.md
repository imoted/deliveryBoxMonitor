...
- 在荷表示用の赤色LED × 2、抵抗 120Ω 1/4W × 2、Groveケーブル 1m（HY2.0-4P）
...
const int LED_PIN[] = {1, 2};     // 在荷表示LED（Grove端子）
...
Grove端子から約1m延長した先に赤色LEDを取り付け、箱に荷物が入っているかを目視で確認できるようにしている。LINE通知が `CNT_THRETHOLD` 回の連続検知を待つのに対し、LEDは目視用なので短いデバウンス（`LED_DEBOUNCE` = 2回）だけかけて直近の計測を反映する。

### 配線

Grove端子（HY2.0-4P）のうち3線を使う。**赤（5V）は使わないので、両端で切り詰めて絶縁すること**（黒と触れるとUSB 5Vの短絡になる）。
...
```
G1(黄) ──[120Ω]────── 1m ──────►|── 上段LED(赤)
G2(白) ──[120Ω]────── 1m ──────►|── 下段LED(赤)
GND(黒) ─────────────  1m ───────┴── 共通カソード
        ↑
    抵抗は基板側（コネクタから5cm以内）に置く
```

- **120Ωは必ず基板側に置く。** 先端で短絡しても電流は 3.3V / 120Ω = 27.5mA に制限され、GPIOの絶対最大定格40mAを下回るのでピンが壊れない。抵抗をLED側に付けると短絡＝GPIO直結GNDで破壊される。
- 赤LED（Vf≒2.0V）で約10mA。ドライブ能力のデフォルト（約20mA）の範囲内。100Ωより下げないこと（82Ωだと短絡時にちょうど40mAに達する）。
- 青・白LEDは Vf≒3.0V で3.3V駆動では光らないため使えない。
- 外付けプルダウンは不要。Hi-Z時はGPIOが電流を吐けないのでLEDは消灯のまま。`setup()` で `pinMode(OUTPUT)` → `digitalWrite(LOW)` している。
- G1/G2はGrove端子のI2C（Port A）だが、`M5.begin()` は `external_rtc` / `external_imu` が有効なときしか `Ex_I2C.begin()` を呼ばない（どちらもデフォルト `false`）ため、通常のGPIOとして使える。

### 点灯条件

計測は `INTERVAL`（10秒）ごとに上段・下段を交互に行うので、1つの箱は20秒に1回計測される。デバウンス2回のため、状態変化がLEDに反映されるまで最大40秒かかる。

`pulseIn` がタイムアウトすると距離が0cmになり、しきい値との単純比較では「在荷」と誤判定される。LEDは `DIST_MIN_VALID`（2cm）未満を無効な計測として捨て、直前の状態を保持する。

> なお、LINE通知側（`judgeBoxOccupied`）は従来どおり0cmを在荷として扱う。センサー断線時に誤通知が出る既存の挙動は変えていない。
...
外付けの在荷表示LEDとは別に、AtomS3 Lite本体のRGB LED（G35）が動作状態を示す。
